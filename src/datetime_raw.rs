...
static DAY_TAB: [i32; 12] = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
static LEAP_DAY_TAB: [i32; 12] = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
...
fn DetermineTimeZoneOffsetInternal(mut tm: &mut pg_tm, tzp: &pg_tz, tp: &mut pg_time_t) -> i32 {
    fn inner(mut tm: &mut pg_tm, tzp: &pg_tz, tp: &mut pg_time_t) -> Result<i32, ()> {
        ...
        // Find the DST time boundary just before or following the target time. We assume that all
        // zones have GMT offsets less than 24 hours, and that DST boundaries can't be closer
        // together than 48 hours, so backing up 24 hours and finding the "next" boundary will
        // work.
        ...
        if res == 0 {
            ...
            *tp = mytime - before_gmtoff;
            return Ok(-(before_gmtoff as i32));
        }
        ...
        // If both before or both after the boundary time, we know what to do. The boundary time
        // itself is considered to be after the transition, which means we can accept aftertime ==
        // boundary in the second case.
        ...
        // It's an invalid or ambiguous time due to timezone transition. In a spring-forward
        // transition, prefer the "before" interpretation; in a fall-back transition, prefer
        // "after".  (We used to define and implement this test as "prefer the standard-time
        // interpretation", but that rule does not help to resolve the behavior when both times are
        // reported as standard time; which does happen, eg Europe/Moscow in Oct 2014.  Also, in
        // some zones such as Europe/Dublin, there is widespread confusion about which time offset
        // is "standard" time, so it's fortunate that our behavior doesn't depend on that.)
